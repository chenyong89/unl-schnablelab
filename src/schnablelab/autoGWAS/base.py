...
class ParseHapmap():
    ...
    def to_df(self, sorting=False):
        ...
        if sorting:
            ...
            df['chrom'] = pd.Categorical(df['chrom'], chrs_ordered,
                                         ordered=True)
        ...
        with open(self.filename) as f:
            ...
            for i in f:
                ...
                try:
                    ...
                    num_homo1, num_homo2, num_hete =\
                        self.count_genotype(j[11:], a1, a2)
                    num_a1, num_a2 = num_homo1*2+num_hete, num_homo2*2+num_hete
                    try:
...
class ParseGWASresults():
    def __init__(self, filename, software, sorting=False, idx_cols=None):
        ...
        if self.software == 'gemma':
            dtype_dict = {'chr': 'str', 'rs': 'str', 'ps': np.int64,
                          'p_lrt': np.float64}
            df = pd.read_csv(self.filename, delim_whitespace=True,
                             usecols=['chr', 'rs', 'ps', 'p_lrt'],
                             dtype=dtype_dict)
            df = df[['rs', 'chr', 'ps', 'p_lrt']]
        if self.software == 'farmcpu':
            dtype_dict = {'Chromosome': 'str', 'SNP': 'str',
                          'Position': np.int64, 'P.value': np.float64}
            df = pd.read_csv(self.filename,
        ...
        if self.software == 'other':
            if self.idx_cols is None:
                raise Exception('specify which columns to use if choosing other gwas tools!')
            ...
            with open(self.filename) as _:
                ...
                pos_col = j[pos_idx]
                pv_col = j[pv_idx]
                dtype_dict = {snp_col: 'str', chr_col: 'str',
                              pos_col: np.int64, pv_col: np.float64}
            cols = [snp_col, chr_col, pos_col, pv_col]
            df = pd.read_csv(self.filename,
                             usecols=cols,
                             dtype=dtype_dict)[cols]
    ...
    def SignificantSNPs(self, p_level=0.05, MeRatio=1, sig_cutoff=None):
        ...
        '''
        cutoff = -np.log10(p_level/(MeRatio * self.num_snp)) \
            if sig_cutoff is None else sig_cutoff
...
def cal_MAF_row(hmp_row):
    ...
    minor_allele, major_allele, = (allele1, allele2) \
        if a1 <= a2 else (allele2, allele1)
