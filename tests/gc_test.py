...
def make_plot(gt_seq, before_seq, after_seq, before_acc, after_acc, fn):
...
def main():
    for seq_type, before_seq, gt_seq in zip(
        ...
        if after_acc > 0.05:
            make_plot(
                gt_seq, before_seq, after_seq, before_acc, after_acc, seq_type
            )
            raise Exception(
                f"correction accuracy for {seq_type} task > 5%, check artifacts for details!"
            )
