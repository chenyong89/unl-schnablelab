...
class RGB():
    ...
    def __init__(self, filename):
        ...
        self.green_idx = (2*self.array_g)/(self.array_r+self.array_b+0.01)
    ...
    def biseg_by_green(self, cutoff=130):
        ...
        '''
        _, thresh = cv2.threshold(self.array_g,
                                  cutoff,
                                  255,
                                  cv2.THRESH_BINARY)
    ...
    def biseg_by_green_idx(self, cutoff=1.12):
        ...
        '''
        _, thresh = cv2.threshold(self.array_gidx,
                                  cutoff,
                                  255,
                                  cv2.THRESH_BINARY)
    ...
    def get_plant_boundry(self, box=None):
        ...
        idx_bottom = self.height - next(x for x, val in enumerate(height[::-1])
                                        if val > 0)
        idx_left = next(x for x, val in enumerate(width) if val > 0)
        idx_right = self.width-next(x for x, val in enumerate(width[::-1])
                                    if val > 0)
...
def call_height(img_fn, zoom_level, threshold=1.12):
    ...
    img_green_idx = (2*img_float[:, :, 1])/(img_float[:, :, 0]+img_float[:, :, 2])
    thresh1 = np.where(img_green_idx > threshold,
                       img_green_idx, 0)
    ...
    _, contours, _ = cv2.findContours(thresh2, cv2.RETR_TREE,
