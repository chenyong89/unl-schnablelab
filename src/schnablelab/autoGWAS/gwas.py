...
def farmcpu(args):
    ...
    with open(r_fn, "w") as f:
        farmcpu_cmd = FarmCPU_header.format(
            pheno=pheno, geno_prefix=geno_prefix, pca=PCA, memo=memo
        )
        f.write(farmcpu_cmd)
...
def gemma_GLM(args):
    ...
    cmd = (
        f"{gemma} -g {meanG} -p {pheno} -a {annoG} -lm 4 -outdir "
...
def mvp_data(args):
    ...
    with open(r_fn, "w") as f:
        f.write(MVP_Data_header.format(hapmap=hmp, output=output_prefix))
    print(f"R file to run on premises: {r_fn}")
...
def mvp(args):
    ...
    with open(r_fn, "w") as f:
        f.write(
            MVP_Run_header.format(pheno=pheno, output_prefix=output_prefix)
        )
    print(f"R file to run on premises: {r_fn}")
