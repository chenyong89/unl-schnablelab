...
def main():
    actions = (
        ...
        ("get_sig_SNPs", "extract significant SNPs from GWAS results"),
...
def plot_manhattan(args):
    ...
    gwas0 = ParseGWASresults(
        gwasfile, opts.software, sorting=opts.sort, idx_cols=opts.idx_cols
    )
    df_plot = gwas0.df
    chr_pos_df = df_plot.groupby("chr").max()
    chr_lens = chr_pos_df["pos"].cumsum().tolist()[0:-1]
    chr_lens.insert(0, 0)
    chr_len_df = pd.DataFrame(
        chr_lens, index=chr_pos_df.index, columns=["chr_offset"]
    ).reset_index()
    ...
    for chr, grp in df_plot.groupby(by="chr"):
        ...
        label_pos = (
            grp["x"].iloc[-1] - (grp["x"].iloc[-1] - grp["x"].iloc[0]) / 2
        )
...
def count_peaks(args):
    ...
    gwas0 = ParseGWASresults(
        gwasfile, opts.software, sorting=opts.sort, idx_cols=idx_cols
    )
    ...
    f0.write(
        "Peak,#_of_SNPs,PeakStartPos,PeakStopPos,PeakLength,MostSigSNP,-log10Pvalue\n"
    )
    for idx, grp in df.groupby("peaks"):
        ...
        f0.write(
            f"{idx},{snp_number}, {st},{ed},{distance},{most_sig_snp},{largest_pvalue:.3f}\n"
        )
    f0.close()
    ...
    print(
        f"results have been saved to {outprefix}.csv and {outprefix}.summary.csv!"
    )
...
def cal_MAF(args):
    ...
    p.add_option("--snp_col_idx", default="0", help="specify the SNP column")
    opts, args = p.parse_args(args)
    ...
    df = (
        pd.read_csv(SNPlist_fn, delim_whitespace=True, header=None)
        if opts.header == "no"
...
def fetch_EVs_from_FarmCPU(args):
    ...
    p.add_option("--col_idx", default="0", help="specify the SNP column")
    opts, args = p.parse_args(args)
    ...
    df = (
        pd.read_csv(SNPlist_csv, delim_whitespace=True, header=None)
        if opts.header == "no"
...
def linked_SNPs(args):
    ...
    if opts.header == "yes":
        df = pd.read_csv(
            SNPlist_fn, delim_whitespace=True, usecols=[opts.col_idx]
        )
    else:
        df = pd.read_csv(
            SNPlist_fn,
    ...
    cmd_local = (
        f"{plink} --bfile {bedprefix} --r2 --ld-snp-list "
...
def extract_geno_from_vcf(args):
    ...
    df = (
        pd.read_csv(snplist, delim_whitespace=True)
        if opts.header == "yes"
    ...
    cmd1 = (
        f"zgrep -f SNP_keys.csv {vcf} > SNPs_keys.tmp.vcf"
        if vcf.endswith("gz")
    ...
    cmd2 = (
        f"zgrep -m 1 -P '#CHROM\tPOS' {vcf} > header.vcf"
        if vcf.endswith("gz")
    ...
    df_geno = pd.read_csv(
        "SNPs_keys.tmp.vcf", delim_whitespace=True, header=None
    )
...
def extract_gene_from_gff3(args):
    ...
    df0 = (
        pd.read_csv(SNPlist, header=None)
        if opts.header == "no"
    ...
    for g in df0.groupby("chr"):
        ...
        for pos in g[1]["pos"]:
            ...
            df2[df2["min_dist"] < 5000].to_csv(
                f"{chrom}_{pos}_genes.csv", index=False, sep="\t"
            )
