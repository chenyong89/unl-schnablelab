...
def main():
    actions = (
        ...
        ("generate_kinship", "generate centered kinship matrix"),
...
def qc_missing(args):
    ...
    outputhmp = Path(inputhmp).name.replace(
        ".hmp", f"_mis{opts.missing_cutoff}.hmp"
    )
...
def qc_hetero(args):
    ...
    (inputhmp,) = args
    outputhmp = Path(inputhmp).name.replace(
        ".hmp", f"_het{opts.het_cutoff}.hmp"
    )
...
def ped_to_bed(args):
    ...
    (ped_prefix,) = args
    cmd_local = (
        f"{plink} --noweb --file {ped_prefix} --make-bed "
        f"--out {ped_prefix}"
    )
    ...
    if not opts.disable_slurm:
        cmd_header = "ml plink"
        cmd = (
            f"plink --noweb --file {ped_prefix} --make-bed --out "
            f"{ped_prefix}"
        )
        slurm_dict = vars(opts)
...
def hapmap_to_vcf(args):
    ...
    (hmpfile,) = args
    cmd_local = (
        f"{tassel} -Xms512m -Xmx10G -fork1 -h {hmpfile} -export"
        " -exportType VCF\n"
    )
    ...
    if not opts.disable_slurm:
        ...
        cmd = (
            f"run_pipeline.pl -Xms512m -Xmx10G -fork1 -h {hmpfile} -export"
            " -exportType VCF\n"
        )
        slurm_dict = vars(opts)
        slurm_dict["cmd_header"] = cmd_header
...
def generate_kinship(args):
    ...
    p.add_option("--output_dir", default=".", help="specify the output dir")
    p.add_option(
        "--disable_slurm",
        default=False,
        action="store_true",
...
def generate_pca(args):
    ...
    cmd_local = (
        f"{tassel} -Xms28g -Xmx29g -fork1 -h {hmpfile} "
    ...
    if not opts.disable_slurm:
        ...
        cmd_slurm = (
            f"run_pipeline.pl -Xms28g -Xmx29g -fork1 -h {hmpfile} "
            f"-PrincipalComponentsPlugin -ncomponents {N} "
...
def independent_SNPs(args):
    ...
    cmd = (
        f"java -Xmx18g -jar {gec} --noweb --effect-number --plink-binary "
...
def reorg_pheno_for_gemma(
    in_dir, out_dir, file_pattern, header=True, sep="\t"
):
    """
    ...
    for fn in input_files:
        df = (
            pd.read_csv(fn, sep=sep)
            if header
            else pd.read_csv(fn, sep=sep, header=None)
        )
        output_fn = fn.name + ".gemma"
        df.iloc[:, 1].to_csv(
            out_path / output_fn, index=False, header=False, na_rep="NA"
        )
