...
FOOTNOTE = (
    "GWAS, High Throughput Genotyping and Phenotyping tools"
...
class OptionParser(OptionP):
    ...
    def add_slurm_opts(self, job_prefix="myjob"):
        ...
        group.add_option(
            "--time", type="int", default=120, help="the maximum running hours"
        )
        group.add_option(
            "--memory",
            type="int",
        ...
        group.add_option(
            "--gpu_model", help="choose gpu_k40, gpu_p100, or gpu_v100"
        )
        group.add_option(
            "--n_cmds_per_slurm",
            type="int",
...
def create_slurm(cmds, kwargs):
    ...
    for range_label, grp in cutlist(cmds, n_slurms):
        jobname = (
            "%s_%s" % (kwargs["job_prefix"], range_label)
            if len(cmds) > 1
            else kwargs["job_prefix"]
        )
