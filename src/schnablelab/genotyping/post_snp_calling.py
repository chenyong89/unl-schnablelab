...
def split_vcf(args):
    ...
    child = subprocess.Popen(
        "wc -l %s" % vcffile, shell=True, stdout=subprocess.PIPE
    )
...
def impute_beagle(args):
    ...
    p.add_option(
        "--parameter_file", help="file including window, overlap parameters"
    )
    ...
    for vcf in vcfs:
        ...
        if opts.parameter_file:
            ...
            cmd = (
                f"beagle -Xmx60G gt={vcf} out={out_fn_path} "
...
def cal_LD(args):
    ...
    for win_snp, win_bp in zip(ld_window[1:], ld_window_bp[1:]):
        prob = 10 / win_snp
        cmd = (
            f"plink {input} --thin {prob} --r2 --ld-window 10 "
...
def summarize_LD(args):
    ...
    *lds, out_prefix = args
    df = pd.concat(
        [
            pd.read_csv(ld, delim_whitespace=True, usecols=[1, 4, 6])
            for ld in lds
        ]
    )
    ...
    plt.legend(
        (line1, line2), ("Original", "Fitted"), frameon=False, numpoints=2
    )
