...
def agg_gvcf(args):
    ...
    for interval, grp in df.groupby("interval"):
        ...
        print(
            f"{grp.shape[0]} gvcf files found for interval {interval}, "
            f"generating the corresponding map file {gvcf_map}..."
        )
        grp[["sm", "fnpath"]].to_csv(
            gvcf_map, header=None, index=False, sep="\t"
        )

        cmd = (
            f"gatk --java-options '-Xmx{mem}g -Xms{mem}g' GenomicsDBImport "
...
def gen_vcf(args):
    ...
    for db in Path(db_dir).glob("*"):
        if db.is_dir():
            ...
            cmd = (
                f"gatk --java-options '-Xmx{mem}g' GenotypeGVCFs "
...
def gatk(args):
    ...
    with open(regions) as f:
        for reg in f:
            ...
            cmd = (
                f"gatk --java-options '-Xmx13G' HaplotypeCaller \\\n"
...
def freebayes(args):
    ...
    p.add_option(
        ...
        default=10000,
        help=(
            "loci where the mapping depth higher than this value "
            "will be ignored"
        ),
    )
    ...
    with open(region) as f:
        for reg in f:
            ...
            reg_fn_vcf_path = out_path / reg_fn_vcf
            cmd = (
                f"freebayes -r {reg} -f {ref} -C 1 -F 0.05 -L {bams} "
