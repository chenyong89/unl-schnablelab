...
def align_pe(args):
    ...
    for (_, r1), (_, r2) in zip(df_R1.iterrows(), df_R2.iterrows()):
        ...
        cmd = f"bwa mem -t {opts.ncpus_per_node} -R {RG} {ref_base} {r1_fn}"\
...
def divide_interval(args):
    ...
    if len(args) == 0:
        sys.exit(not p.print_help())
    fai_fn, fn_out, = args
    df = pd.read_csv(fai_fn, header=None, delim_whitespace=True,
                     usecols=[0, 1])
    ...
    with open(fn_out, 'w') as f:
        for _, row in df.iterrows():
            ...
            if opts.num_chunk == 1:
                ...
                for interval in pd.qcut(np.arange(length), opts.num_chunk,
                                        precision=0).categories:
...
def trim_paired(args):
    ...
    for r1_fn, r2_fn in zip(r1_fns, r2_fns):
        ...
        cmd = f'java -jar $TM_HOME/trimmomatic.jar PE -phred33 {r1_fn} '\
...
def trim_single(args):
    ...
    for fn in fns:
        ...
        cmd = f'java -jar $TM_HOME/trimmomatic.jar SE -phred33 {fn} '\
