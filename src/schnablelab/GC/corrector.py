...
class Correction(object):
    ...
    def get_corrected(self):
        ...
        for __, grp in rolling_geno.groupby(grouper):
            ...
            if geno == 1:
                ...
                if self.judge_h_island(h_score_island):
                    ...
                    fix_slide = FixSliding(
                        self.seq_num, rolling_geno, grp.index
                    )
                    fix_slide.case1()
                    fix_slide.case2()
                    fix_slide.case3()
        ...
        corrected = rolling_geno.where(
            ~rolling_geno.isna(), other=self.seq_num
        ).astype("int")
        return corrected
        ...
        if ab_count > self.win_size // 2:
            ...
            d = {
                ...
                for (key, value) in zip(
                    [0, 1, 2], [a_ex_prob, h_ex_prob, b_ex_prob]
                )
            }
...
def correct(args):
    ...
    p.add_option("--random_seed", type="int", help="specify random seed")
    q = OptionGroup(p, "output options")
    p.add_option_group(q)
    q.add_option("--opp", help="the prefix of the output filenames")
    q.add_option(
        "--logfile",
    ...
    q.add_option(
        ...
        action="store_true",
        help=(
            "generate a tmp file containing both original and "
    ...
    opf = (
        inputmatrix.rsplit(".", 1)[0] + ".corrected.map"
        if opts.opp is None
        else "{}.map".format(opts.opp)
    )
    if Path(opf).exists():
    ...
    map_reader = pd.read_csv(
        inputmatrix, delim_whitespace=True, index_col=[0, 1], iterator=True
    )
    tmp_chr_list = []
    for chrom in chr_order:
        ...
        logging.debug(
            "{} contains {} markers and {} samples.".format(
        ...
        for sm in df_chr_tmp:
            ...
            if seq_no_idx_num.shape[0] <= cargs.win_size:
                logging.debug(
                    "num of markers smaller than predefined window "
                    "size, skip..."
                )
                ...
                corrected_n = get_corrected_num(seq_no_idx_num, corrected_seq)
                round_n = 2
    ...
    if opts.debug:
        ...
        df_uncorrected = pd.read_csv(
            inputmatrix, delim_whitespace=True, index_col=[0, 1]
        )
...
def cleanup(args):
    ...
    df.applymap(lambda x: x.split("(")[0]).to_csv(
        outputmatrix, sep="\t", index=True
    )
    print("Done!")
...
def format(args):
    ...
    r.add_option(
        "--mstmap_pop_type",
        help=(
            "values can be DH or RILd where d is a natural number. "
    ...
    r.add_option(
        ...
        default=0.000001,
        help=(
            "specifies the threshold to be used for clustering the "
            "markers into LGs"
        ),
    )
    r.add_option(
        "--no_map_dist", default=15, help="check mstmap manual for details"
    )
    r.add_option(
        "--no_map_size", default=5, help="check mstmap manual for details"
    )
    r.add_option(
        "--missing_threshold",
        default=0.4,
        help=(
            "any marker with more than this value will be removed "
    ...
    r.add_option(
        ...
        choices=("yes", "no"),
        help=(
            "if yes, MSTmap will try to estimate missing data "
    ...
    if opts.mstmap:
        if not opts.mstmap_pop_type:
            ...
            sys.exit(1)
        if not (
            opts.mstmap_pop_type.startswith("RIL")
        ...
        df["locus_name"] = (
            df.iloc[:, 0].astype("str") + "_" + df.iloc[:, 1].astype("str")
        )
        df = df[cols]
        print(df.head())
        ...
        f1.write(
            MST_header.format(
                opts.mstmap_pop_type,
                opts.population_name,
                opts.distance_function,
                opts.cut_off_p_value,
                opts.no_map_dist,
                opts.no_map_size,
                opts.missing_threshold,
                opts.estimation_before_clustering,
                opts.detect_bad_data,
    ...
    if opts.joinmap:
        ...
        df.to_excel(opf)
        print(
            "Done! Now you can load the genotype data into a JoinMap "
