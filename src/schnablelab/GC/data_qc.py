...
from .base import eprint, getChunk, get_blocks, sort_merge_sort, bin_markers
...
def qc_missing(args):
    ...
    p.add_option(
        ...
        type="float",
        help=(
            "SNPs with missing rate higher than this cutoff will " "be removed"
        ),
    )
    p.add_option(
        "--cutoff_sample",
        type="float",
        help=(
            "if --rm_bad_samples is True, sample whose missing "
    ...
    q.add_option(
        "--homo1", default="A", help="character for homozygous genotype"
    )
    q.add_option(
        "--homo2",
        default="B",
    ...
    q.add_option(
        "--hete", default="X", help="character for heterozygous genotype"
    )
    ...
    for chrom in chr_order:
        ...
        good_rates = (
            df_chr_tmp_num.apply(lambda x: (x == 9).sum() / sample_num, axis=1)
            <= opts.cutoff_snp
        )
    ...
    print(
        f"{n0_snp} SNP markers before QC; "
        f"{n1_snp}({pct:.1f}%) markers left after QC"
    )

    if opts.cutoff_sample:
        good_samples = (
            df1.apply(lambda x: (x == opts.missing).sum() / n1_snp, axis=0)
            <= opts.cutoff_sample
        )
        ...
        print(
            f"{n0_sm} samples before quality control; "
...
def qc_sd(args):
    ...
    p.add_option(
        ...
        type="float",
        help=(
            "set the chi square test cutoff between 0 "
    ...
    for chrom in chr_order:
        ...
        ob0, ob2 = (
            (df_chr_tmp_num == 0).sum(axis=1),
            (df_chr_tmp_num == 2).sum(axis=1),
        )
        obsum = ob0 + ob2
        exp0, exp2 = (
            (obsum * 0.75, obsum * 0.25)
            if opts.population == "BCFn"
            else (obsum * 0.5, obsum * 0.5)
        )
        df_chi = pd.DataFrame(
            dict(zip(["ob0", "ob2", "exp0", "exp2"], [ob0, ob2, exp0, exp2]))
        )
        min_cond = ((df_chi["ob0"] > 5) & (df_chi["ob2"] > 5)).values
        pval_cond = (
            chisquare(
                [df_chi["ob0"], df_chi["ob2"]],
                [df_chi["exp0"], df_chi["exp2"]],
            ).pvalue
            >= opts.sig_cutoff
        )
...
def qc_hetero(args):
    ...
    r.add_option(
        ...
        type="int",
        help=(
            "number of homozygous between two heterozygous in "
    ...
    r.add_option(
        ...
        type="int",
        help=(
            "number of continuous homozygous within the read "
    ...
    for chrom in chr_order:
        ...
        for sm in df_chr_tmp_num:
            geno_grouper = (
                (df_chr_tmp_num[sm].diff(1) != 0).astype("int").cumsum()
            )
            idx, geno, lens = [], [], []
            ...
            type_grouper = (
                (df_grp_geno["type"].diff(1) != 0).astype("int").cumsum()
            )
        ...
        if chr_bin_ids:
            ...
            for idx_block in merged_bin_ids:
                ...
                dropping_index = list(
                    pd.concat([missings, heteros], axis=1)
                    .sort_values([0, 1])
                    .index[1:]
                )
...
def vcf2map(args):
    ...
    p.add_option(
        "--logfile", default="GC.vcf2map.info", help="specify the log file"
    )
    opts, args = p.parse_args(args)
    if len(args) != 2:
        sys.exit(not p.print_help())
...
def qc_dup(args):
    ...
    p.add_option(
        ...
        type="int",
        help=(
            "number of different genotypes between two consecutive "
