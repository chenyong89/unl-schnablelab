...
def stat(original, precorrect, corrected):
    ...
    print(
        "this population contain total % samples: \n%s"
        % (len(samples), samples)
    )
    ...
    for k in ori_ls:
        for l in k:
            ...
            if l == "-":
                miss += 1
    print(
        "\t%s (%s) homozygous genotypes"
        % (homo_base, homo_base / float(total_bases))
    )
    print(
        "\t%s (%s) heterozygous genotypes"
        % (hete_base, hete_base / float(total_bases))
    )
    ...
    print(
        "%s genotypes needed to correct in homozygous region"
        % (homo_miss + homo_fals)
    )
    print(
        "\t%s missing genotypes, %s wrong genotypes" % (homo_miss, homo_fals)
    )
    print(
        "%s genotypes needed to correct in heterozygous region"
        % (hete_miss + hete_fals)
    )
    print(
        "\t%s missing genotypes, %s wrong genotypes" % (hete_miss, hete_fals)
    )
    ...
    print(
        "%s (correct rate: %s) genotypes were corrected, %s right (right rate:\
%s), %s wrong (wrong rate: %s)"
    ...
    print(
        "\t%s genotypes were corrected in homo region, %s (%s)right, %s (%s)wrong"
        % (
            cor_homo,
            cor_r_homo,
            cor_r_homo / float(cor_homo),
            cor_w_homo,
            cor_w_homo / float(cor_homo),
        )
    )
    print(
        "\t%s genotypes were corrected in hete region, %s (%s)right, %s (%s)wrong"
        % (
            cor_hete,
            cor_r_hete,
            cor_r_hete / float(cor_hete),
            cor_w_hete,
            cor_w_hete / float(cor_hete),
        )
    )
