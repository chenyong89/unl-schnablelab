    ...
    if opts.range == "all":
        ...
        if end <= len(alljobs):
            for i in alljobs[start - 1 : end]:
                ...
                call(i, shell=True)
            print(
                f"{len(alljobs[start - 1: end])} of total {len(alljobs)}"
...
def cancel(args):
    ...
    if len(args) != 0:
        sys.exit(not p.print_help())
    myjobs = (
        Popen("squeue -u cmiao", shell=True, stdout=subprocess.PIPE)
        .communicate()[0]
        .decode("utf-8")
    )
