...
def main():
    actions = (("cpu", "request cpu node"), ("gpu", "request gpu node"))
...
def cpu(args):
    ...
    p.add_option(
        "--memory", default="10240", help="specify memory [default: %default]"
    )
    p.add_option(
        "--time",
        default="20",
    ...
    if len(args) == 0:
        ...
        cmd = (
            f"srun --partition={opts.partition} --mem-per-cpu={opts.memory}"
...
def gpu(args):
    ...
    if len(args) == 0:
        print("add --help to see options.\n")
        cmd = (
            f"srun --partition=schnablelab --gres=gpu "
