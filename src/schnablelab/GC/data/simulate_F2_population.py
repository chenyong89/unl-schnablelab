...
def main(popu_n, chr_n, eachr_len, mark_dens, file_name, ho_a, ho_b, he_x):
    ...
    genotype_info = gen_F2_genotype(
        chr_n, eachr_len, mark_dens, popu_n, ho_a, ho_b, he_x
    )
    f0 = open(file_name, "w")
    f0.write(first_line + "\n")
    need_trans_ls = []
...
def gen_F2_genotype(chr_n, eachr_len, mark_dens, popu_n, ho_a, ho_b, he_x):
    ...
    for i in range(popu_n):
        first_set_geno, second_set_geno = gen_F1_genotype(
            chr_n, eachr_len, mark_dens, ho_a, ho_b
        )
...
def gen_F1_genotype(chr_n, eachr_len, mark_dens, ho_a, ho_b):
    ...
    for c1, c2, n, idx in zip(
        first_set_geno, second_set_geno, exchange_n, genoseq_idx
    ):
        if n == 0:
            continue
        else:
...
if __name__ == "__main__":
    main(
        120,
        6,
        [30, 25, 16, 30, 15, 27, 20, 23],
        0.04,
        "F2_120i_6chr_0.03dens.map",
        "A",
        "B",
        "X",
    )
